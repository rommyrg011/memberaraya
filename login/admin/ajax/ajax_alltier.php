...
// Basis query untuk semua Tier yang hanya aktif
$baseQuery = "SELECT * FROM member WHERE status = 'Aktif'";
...
$sel = mysqli_query($koneksi, "SELECT COUNT(*) AS allcount FROM member WHERE status = 'Aktif' " . $searchQuery);
if (!$sel) {
    http_response_code(500);
    echo json_encode(['error' => 'Query error: ' . mysqli_error($koneksi)]);
    exit;
