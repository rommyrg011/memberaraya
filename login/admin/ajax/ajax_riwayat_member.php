...
// Ubah klausa WHERE untuk hanya menghitung member yang tidak aktif
$sel = mysqli_query($koneksi, "SELECT COUNT(*) as allcount FROM member WHERE status = 'Tidak Aktif'");
if (!$sel) {
    http_response_code(500);
...
// Ubah klausa WHERE 'status = 'Aktif'' menjadi 'status = 'Tidak Aktif''
$searchQuery = " AND status = 'Tidak Aktif' ";
if ($searchValue != '') {
...
## Ambil data
// Ubah klausa WHERE 'status = 'Aktif'' menjadi 'status = 'Tidak Aktif''
$query = "SELECT * FROM member WHERE status = 'Tidak Aktif' ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
