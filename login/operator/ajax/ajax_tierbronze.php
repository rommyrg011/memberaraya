...
// Basis query untuk Tier Bronze yang hanya aktif
$baseQuery = "SELECT * FROM member WHERE tier = 'Bronze' AND status = 'Aktif'";

## Query untuk total record (tanpa filter)
$sel = mysqli_query($koneksi, "SELECT COUNT(*) as allcount FROM member WHERE tier = 'Bronze' AND status = 'Aktif'");
if (!$sel) {
...
$sel = mysqli_query($koneksi, "SELECT COUNT(*) AS allcount FROM member WHERE tier = 'Bronze' AND status = 'Aktif' " . $searchQuery);
if (!$sel) {
    http_response_code(500);
    echo json_encode(['error' => 'Query error: ' . mysqli_error($koneksi)]);
    exit;
